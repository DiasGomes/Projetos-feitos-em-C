...
void *threadFilosofo (void *arg)
{
  ...
  {
    medita (i) ;
    sem_wait (&saleiro) ;	// lock no saleiro
    printf ("%sF%d pegou o saleiro\n", space[i], i) ;
    pega_hashi (i, i) ;
    pega_hashi (i, (i+1) % NUMFILO) ;
    sem_post(&saleiro);		// libera o saleiro
    printf ("%sF%d liberou o saleiro\n", space[i], i) ;
    come (i) ;
    larga_hashi (i, i) ;
...
int main (int argc, char *argv[])
{
  ...
  // inicializa um saleiro
  //sem_init (&saleiro, 0, 1) ;
  // inicializa N - 1 saleiro
